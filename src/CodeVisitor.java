...
public class CodeVisitor extends BasicParserBaseVisitor<String> {
	...
	private LinkedList<String> currentf;
	...
	public String visitProg(BasicParser.ProgContext ctx) {
		...
		currentf = new LinkedList<String>();
		...
		if (currentf.size() != 0) {
			System.out.println("currengf should be clear");
			currentf.clear();
		} else {
		...
		if (currentScope.getSpace() != 0) {
			...
			System.out.println("main scope sapce: " + scopeSpace);
		}
	...
	@Override
	public String visitDowhile(@NotNull BasicParser.DowhileContext ctx) {
	...
	@Override
	public String visitForloop(@NotNull BasicParser.ForloopContext ctx){
...
.L2:
	...
	ble	.L3
	add	sp, fp, #0*/
		for(BasicParser.StatementContext st : ctx.statement()){
			System.out.println("OIOIOIOIOIOIOIOI st.getText() = " + st.getText());
		}
		visit(ctx.statement(0)); //declaration int i = 0;
		String label0 = labelMan.nextLabel(); //label for the body
		insMan.addInstruction(label0 + ":");
		System.out.println("((((((((((((((((((((((((stat2: " + visit(ctx.statement(2))); // body of the loop
		System.out.println("((((((((((((((((((((((((");
		System.out.println("((((((((((((((((((((((((stat1: " + visit(ctx.statement(1))); // 'increment' statement
		System.out.println("((((((((((((((((((((((((");

		System.out.println("EXPR: " + visit(ctx.expression()));

		//stack.getOffsetGlobal();
		return null;
	}
	...
	public String visitFunction(@NotNull BasicParser.FunctionContext ctx) {
		...
		currentf.add(ctx.IDENT().getText());
		System.out.println("test func para space : "
				+ funcParaSpace.get(currentf.getLast()));
		System.out.println("current scope space : " + currentScope.getSpace());
		Scope save = currentScope;
		currentScope = currentScope.nextChild();
		// stack.JustIncreaseSp(-funcParaSpace.get(currentf)-4);
		System.out.println("Function current scope space: "
				+ currentScope.getSpace());
		System.out.println("Function parameter space: "
				+ funcParaSpace.get(currentf.getLast()));
		stack.enterScope(currentScope.getSpace() + 4);
		insMan.addInstruction(labelMan.newFuncLabel(ctx.IDENT().getText()));
		insMan.addInstruction("PUSH {lr}");
		if ((currentScope.getSpace() - funcParaSpace.get(currentf.getLast())) != 0) {
			insMan.addInstruction("SUB sp, sp, #"
					+ (currentScope.getSpace() - funcParaSpace.get(currentf.getLast())));
		}
	...
	public String visitArgumentList(@NotNull BasicParser.ArgumentListContext ctx) {
		...
		while (li.hasPrevious()) {
			...
			Type thistype = functable.lookupCurrLevelOnly(currentf.getLast())
	...
	public String visitReturn(@NotNull BasicParser.ReturnContext ctx) {
		System.out.println("visitReturn: " + ctx.getText());
		String reg = visit(ctx.expression());
		insMan.addInstruction(new Instruction("MOV", "r0", reg));
		//System.out.println("currentf: " + currentf);
		System.out.println("currentScope.getSpace(): "
				+ currentScope.getSpace());
		System.out.println("funcParaSpace.get(currentf.getlast()): "
				+ funcParaSpace.get(currentf.getLast()));
		int offset = -stack.getSp()-(funcParaSpace.get(currentf.getLast())+4);
		System.out.println("offset= " + offset);
		if (offset != 0) {
